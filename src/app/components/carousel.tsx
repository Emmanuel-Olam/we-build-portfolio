...
const ChakraCarousel = ({ children, gap }: { children: any[] | any; gap: number }) => {
...
const Slider = ({
  ...
  const [ref, _width] = useBoundingRect();
  const { width } = _width as { width: number };
...
const Track = ({
  ...
  const node = useRef<HTMLDivElement | HTMLUListElement | HTMLLIElement | null>(null);
  ...
  const handleClick = useCallback(
    (event: MouseEvent) =>
      node.current?.contains(event.target as any)
        ? setTrackIsActive(true)
  ...
  return (
    <>
      {itemWidth && (
        <VStack ref={node as React.RefObject<HTMLDivElement>} spacing={5} alignItems="stretch">
          <MotionFlex
